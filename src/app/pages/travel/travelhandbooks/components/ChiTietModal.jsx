...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/travelhandbooks/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT(`api/v1/travelhandbooks/${id}`, formData) : await requestPOST(`api/v1/travelhandbooks`, formData);
      if (res) {
