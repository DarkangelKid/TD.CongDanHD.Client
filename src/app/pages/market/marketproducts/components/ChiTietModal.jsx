...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/marketproducts/${id}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await requestPOST(`api/v1/marketcategories/search`, {
        pageNumber: 1,
        pageSize: 1000,
        orderBy: ['name'],
      });
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT(`api/v1/marketproducts/${id}`, formData) : await requestPOST(`api/v1/marketproducts`, formData);
      if (res) {
          ...
          {!loadding && (
            <Form form={form} layout='vertical' /* initialValues={initData} */ autoComplete='off'>
              <div className='row'>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Danh mục' name='marketCategoryId'>
                    <Select
                      showSearch
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Nhãn hiệu' name='brand'>
                    <Input placeholder='' />
                  </FormItem>
                </div>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Đơn vị' name='packaging'>
                    <Input placeholder='' />
                  </FormItem>
                </div>
