...
import {Form, Input, Select, Spin, InputNumber, Cascader} from 'antd';
...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/ecommercecategories/${id}`);
      if (res && res.data) {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      if (formData.parentsId && formData.parentsId.length > 0) {
        formData.parentId = formData.parentsId[formData.parentsId.length - 1];
      }

      const res = id ? await requestPUT(`api/v1/ecommercecategories/${id}`, formData) : await requestPOST(`api/v1/ecommercecategories`, formData);
      if (res) {
            ...
            <Form form={form} layout='vertical' /* initialValues={initData} */ autoComplete='off'>
              <div className='row'>
                <div className='col-xl-6 col-lg-6'>
                  <Form.Item label='Danh mục' name='parentsId'>
                    <Cascader
                      fieldNames={{
                        label: 'name',
                        value: 'id',
                ...
                <div className='col-xl-12 col-lg-12'>
                  <FormItem label='Mô tả chung' name='description'>
                    <TextArea rows={4} placeholder='' />
                  </FormItem>
                </div>
                <div className='col-xl-12 col-lg-12'>
                  <FormItem label='Mô tả chi tiết' name='metaTitle'>
                    <TextArea rows={4} placeholder='' />
                  </FormItem>
                </div>
              </div>
