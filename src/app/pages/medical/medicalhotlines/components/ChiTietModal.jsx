...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/medicalhotlines/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT(`api/v1/medicalhotlines/${id}`, formData) : await requestPOST(`api/v1/medicalhotlines`, formData);
      if (res) {
