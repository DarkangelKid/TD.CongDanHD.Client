...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/carutilities/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT(`api/v1/carutilities/${id}`, formData) : await requestPOST(`api/v1/carutilities`, formData);
      if (res) {
          ...
          {!loadding && (
            <Form form={form} layout='vertical' /* initialValues={initData} */ autoComplete='off'>
              <div className='row'>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Đường dẫn ảnh' name='image'>
                    <Input placeholder='' />
                  </FormItem>
                </div>
