...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/enterpriseforumcategories/${id}`);

      if (res && res.data) {
        form.setFieldsValue(res.data);
        setImage(handleImage(res.data?.image ?? '', FILE_URL));
        setIcon(handleImage(res.data?.icon ?? '', FILE_URL));
      }
      setLoadding(false);
    };
    if (id) {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      icon.forEach((i) => {
        if (i.response) {
          arrIcon.push(i.response.data[0].url);
        } else {
          arrIcon.push(i.path);
        }
      });
      form.setFieldsValue({icon: arrIcon.join('##')});
      ...
      const res = id
        ? await requestPUT(`api/v1/enterpriseforumcategories/${id}`, formData)
        : await requestPOST(`api/v1/enterpriseforumcategories`, formData);
      if (res) {
        ...
        <Spin spinning={loadding}>
          {!loadding && (
            <Form form={form} layout='vertical' /* initialValues={initData} */ autoComplete='off'>
              ...
              <div className='row '>
                <div className='col col-xl-12'>
                  <FormItem label='Ảnh icon'>
                    <ImageUpload
                      URL={`${API_URL}/api/v1/attachments`}
                      fileList={icon}
                      onChange={(e) => setIcon(e.fileList)}
                      headers={{
                        Authorization: `Bearer ${token}`,
                      }}
                    />
