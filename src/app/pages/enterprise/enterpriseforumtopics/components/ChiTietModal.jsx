...
const ModalItem = (props) => {
  ...
  const [enterpriseforumcategories, setEnterpriseforumcategories] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/enterpriseforumtopics/${id}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (res && res.data) setEnterpriseforumcategories(res.data);
    };
    fetchData();
    return () => {};
  }, []);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT(`api/v1/enterpriseforumtopics/${id}`, formData) : await requestPOST(`api/v1/enterpriseforumtopics`, formData);
      if (res) {
              ...
              <div className='row'>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Lĩnh vực' name='enterpriseForumCategoryId'>
                    ...
                    >
                      {enterpriseforumcategories.map((item) => {
                        return (
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='UserName' name='userName' rules={[{required: true, message: 'Không được để trống!'}]}>
                    <Input placeholder='' />
                  </FormItem>
                </div>
